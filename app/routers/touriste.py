...
# 🔒 1. Importation du modèle User et de ta fonction de sécurité
...
def update_tourist(
    tourist_id: int, 
    tourist_data: schemas.ItemUpdate, 
    db: Session = Depends(get_db),
):
...
# @router.delete("/{tourist_id}", dependencies=[Depends(RoleChecker(["admin"]))])
